...
이를 활용해 고객별 물품별로 구해진 재구매율을 모두 더해 전체 대비 재구매한 것의 비율<br>
...
## 결과
-> 그래서 재구매율이 높은 고객이 몇, 낮은 사람이 몇 정도 나왔다는 결과(시각화를 하던가)를 알려주면 좋을 듯 함


# INSIGHT
## Clustering
### 사용 변수
약 25개의 조건을 정리해놓은 많은 (성별, 연령대 등의) 피쳐 중 <br>
구매상품수 | 첫-마지막 | 구매건수 | 구매항목수 | 영수증평균개수 | 구매금액평균 | 구매간격 | 구매_평균항목수 | 총구매금액 | 총영수증개수 | 최다구매물품 | 재구매율<br>
→ 중요변수 12개를 추출하여 군집화 진행<br>
...
# 마케팅 제안
## 추천 시스템 기반
각 군집 별 특성을 파악하여 군집 내 재구매율이 높은 고객군에서 구매 예측된 물품을 재구매율이 낮은 고객군에게 가장 효율적인 추천시스템으로 적용해보기로 함 <br>
