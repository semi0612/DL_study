...
→ C군집에 비하여 재구매율이 낮은 편이기 때문에 현재 구매 물건으로 추천을 하기보다, 조금이라도 재구매율을 높일 수 있도록 재구매 가능성이 높게 예측된 고객군의 구매물품을 분석(예측)하여 추천<br>
→ 사용자를 기반으로 물품 추천<br>

- C군집 <br>
→ A, B군집에 비하여 재구매율이 높은 편이므로 군집전체의 구매물품의 연관규칙을 분석하여 추쳔<br>
→ 아이템 기반 추천 시스템 <br>
→ 장바구니 분석(연관규칙분석; apriori algorithm;) 활용 <br>

## 
A, B 각 군집의 특성을 파악하여 군집 내 재구매율이 높은 고객군에서 구매 예측된 물품을 재구매율이 낮은 고객군에게 가장 효율적인 추천시스템으로 적용. <br>
